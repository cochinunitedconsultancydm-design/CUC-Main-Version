...
""";
  code = code.replaceAll(targetAdd, replacementAdd);

  // 2. _showForm controllers
  final targetControllers = """
    final email = TextEditingController(text: user?['email']);
    final password = TextEditingController();
    String role = user?['role'] ?? 'staff';
""";
  final replacementControllers = """
    final email = TextEditingController(text: user?['email']);
    final password = TextEditingController();
    ...
    String role = user?['role'] ?? 'staff';
""";
  code = code.replaceAll(targetControllers, replacementControllers);
                    ...
                    child: SingleChildScrollView(
                      child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 16),
""";
  code = code.replaceAll(targetUI, replacementUI);

  // We need to close the extra `SingleChildScrollView(Column(...))` we added, just before the buttons.
  final targetButtons = """
                  const SizedBox(height: 32),
                  Row(
                    mainAxisAlignment: MainAxisAlignment.end,
""";
  final replacementButtons = """
                        ],
                      ),
                    ),
                  ),
                  ...
                  Row(
                    mainAxisAlignment: MainAxisAlignment.end,
""";
  code = code.replaceAll(targetButtons, replacementButtons);
...
""";
  code = code.replaceAll(targetSave, replacementSave);
...
""";
  code = code.replaceAll(targetUpdate, replacementUpdate);
