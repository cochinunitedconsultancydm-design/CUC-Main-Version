  ...
  for (var entry in errorsByFile.entries) {
    ...
    for (var ln in linesToFix) {
      if (ln - 1 >= 0 && ln - 1 < lines.length) {
        ...
        text = text.replaceAll("getDateTimeInUtc()", "toString()");
        
        if (text.contains('client_id:')) {
          text = text.replaceAll(RegExp(r'client_id:.*'), "client_id: int.tryParse(license.clientId.toString()) ?? 0,");
        }
        if (text.contains('license_type_id:')) {
          text = text.replaceAll(RegExp(r'license_type_id:.*'), "license_type_id: int.tryParse(license.licenseTypeId.toString()) ?? 0,");
        }
